...
$app->get('/api/patients/{lids}', function($request){
	$lu_ids = explode('::',$request->getAttribute('lids'));
	$lab_id = trim($lu_ids[0]);
	//$uid = trim($lu_ids[1]);
	$qry="select p.id, p.email_id, concat(p.title,' ',p.first_name,' ',p.last_name) as fullname, p.address, p.city, p.mobile, date_format(p.updated,'%b %d, %Y %H:%i:%s') as updated from bl_patients p where p.status='ACTIVE'";
	try{
		$lab_db = new lab_db();
...
$app->get('/api/patient/{lids}', function($request){
	$lu_ids = explode('::',$request->getAttribute('lids'));
	$lab_id = trim($lu_ids[0]);
	$pat_id = trim($lu_ids[1]);
	$qry="select p.id, p.email_id, p.title, p.first_name, p.last_name, p.address, p.city, p.mobile, p.status, date_format(p.updated,'%b %d, %Y %H:%i:%s') as updated from bl_patients p where p.status='ACTIVE' and p.id='".$pat_id."'";
	try{
		$lab_db = new lab_db();
		$lab_db = $lab_db->connect($lab_id);
		if($lab_db==null) {
...
$app->post('/api/patient/{lids}', function($request, $response, $args){
	...
	$qry="insert into bl_patients (id, title, first_name, last_name, email_id, address, city, mobile, status) values (:newid, :title, :first_name, :last_name, :email_id, :address, :city, :mobile, :status)";
	try{
		$lab_db = new lab_db();
		$lab_db = $lab_db->connect($lab_id);
		if($lab_db==null) {
...
$app->put('/api/patient/{lids}', function($request, $response, $args){
	$lu_ids = explode('::',$request->getAttribute('lids'));
	$lab_id = trim($lu_ids[0]);
	$pat_id = trim($lu_ids[1]);
	...
	$uQry = "update bl_patients set title = :title, first_name = :fname, last_name = :lname, email_id = :emailid, address = :address, city = :city, mobile = :mobile, status = :status where id='".$pat_id."'";
	try{
...
$app->delete('/api/patient/{lids}', function($request){
	$lu_ids = explode('::',$request->getAttribute('lids'));
	$lab_id = trim($lu_ids[0]);
	$pat_id = trim($lu_ids[1]);
	$uQry="update bl_patients set status='DELETED' where id='".$pat_id."'";
	try{
		$lab_db = new lab_db();
		$lab_db = $lab_db->connect($lab_id);
		if($lab_db==null) {
